...
public class ViewPrincipal extends JFrame {
	...
	public ViewPrincipal(String nome, String lotacao, String login, ColUsuarios usuarios) {
		...
		if(existeXML(armazenamentoPontos)){
			...
			//Atualizando e Ordenando minha cole��o INICIALMENTE
			...
			//Seto todos os nomes e logins na cole��o
			pontosDiarios.setAllNomes(nome, login);
			//Seto mes e ano na cole��o
			pontosDiarios.setAllMesAno(retornaMes(), retornaAno());
			//Seto a lotacao na cole��o
		...
		//Procurando por algum registro de Ponto do Usu�rio Autenticado na DATA DE HOJE
		PontoDiario RecebePontoDoUsuario = pontosDiarios.retornaUsuarioPelaData(dia);
		
		//DESABILITANDO OS BOT�ES CASO O USU�RIO J� TENHA BATIDO O PONTO
		if(RecebePontoDoUsuario.getHoraEntrada().equals("")){
			...
			if(RecebePontoDoUsuario.getHoraSaida().equals("")){
				...
				btnFechar.setText("SA�DA MARCADA");
			}
		}
		...
		btnFechar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				//Pegando a frequ�ncia do usu�rio pela data de hoje
				PontoDiario novoPonto = pontosDiarios.retornaUsuarioPelaData(dia);
				
				if(!novoPonto.getHoraEntrada().equals("")){
					//Aqui eu Pego a hora atual, Seto a hora da sa�da, coloco a assinatura do usuario e salvo as informa��es em XML
					...
					JOptionPane.showMessageDialog(null, "VOC� PRECISA ABRIR O PONTO PRIMEIRO!");
				}
